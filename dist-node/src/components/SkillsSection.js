...
const SkillsSection = () => {
  ...
  <RiGlobalLine /> Web Development
          ...
          <div className="skills__info">
            {devSkills.map((skill, index) => (<div className="skills__data" key={index}>
  ...
  <RiCodeSSlashLine /> Programming Languages
          ...
          <div className="skills__info">
           {fundamentals.map((skill, index) => (<div className="skills__data" key={index}>
    <div className="skills__blob">
      <img src={skill.img} alt={`${Array.isArray(skill.name) ? skill.name.join(' ') : skill.name} icon`}/>
    </div>
    <h3 className="skills__name">
      {(Array.isArray(skill.name) ? skill.name : skill.name.split(' ')).map((part, i) => (<React.Fragment key={i}>
          {i > 0 && <br />}
          {part}
        </React.Fragment>))}
    </h3>
